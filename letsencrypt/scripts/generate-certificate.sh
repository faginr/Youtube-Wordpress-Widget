...
LEGO_BIN=/home/rex/Desktop/Code/WordPress/Ytube/letsencrypt/lego
CERTIFICATES_DIR=/home/rex/Desktop/Code/WordPress/Ytube/letsencrypt/certificates
...
is_apache_installed() {
    if [ -d "/home/rex/Desktop/Code/WordPress/Ytube/apache2" ]; then
        true
    else
        false
    fi
...
stop_server() {
    /home/rex/Desktop/Code/WordPress/Ytube/ctlscript.sh stop "$(server_type)"
}

start_server() {
    /home/rex/Desktop/Code/WordPress/Ytube/ctlscript.sh start "$(server_type)"
}

restart_server() {
    /home/rex/Desktop/Code/WordPress/Ytube/ctlscript.sh restart "$(server_type)"
}

is_varnish_enabled() {
    if [[ -f "/home/rex/Desktop/Code/WordPress/Ytube/varnish/scripts/ctl.sh" ]]; then
        true
    else
        false
    fi
...
is_varnish_running() {
    if is_varnish_enabled && "/home/rex/Desktop/Code/WordPress/Ytube/ctlscript.sh" status varnish 2> /dev/null | grep "already running" > /dev/null 2>&1 ; then
        true
    else
        false
    fi
...
stop_varnish() {
    "/home/rex/Desktop/Code/WordPress/Ytube/ctlscript.sh" stop "varnish"
}

start_varnish() {
    "/home/rex/Desktop/Code/WordPress/Ytube/ctlscript.sh" start "varnish"
}
...
web_server_conf_dir() {
    if is_apache_installed; then
        echo "/home/rex/Desktop/Code/WordPress/Ytube/apache2/conf"
    else
        echo "/home/rex/Desktop/Code/WordPress/Ytube/nginx/conf"
    fi
}
...
modify_configuration() {
    if is_apache_installed; then
        sed -i "s;\s*SSLCertificateFile\s.*;  SSLCertificateFile \"/home/rex/Desktop/Code/WordPress/Ytube/apache2/conf/${domains[0]}.crt\";g" "$(web_server_conf_dir)/bitnami/bitnami.conf"
        sed -i "s;\s*SSLCertificateKeyFile\s.*;  SSLCertificateKeyFile \"/home/rex/Desktop/Code/WordPress/Ytube/apache2/conf/${domains[0]}.key\";g" "$(web_server_conf_dir)/bitnami/bitnami.conf"
    else
...
EOF
    ...
    if is_apache_installed; then
        server_restart_command="/home/rex/Desktop/Code/WordPress/Ytube/apache2/bin/httpd -f /home/rex/Desktop/Code/WordPress/Ytube/apache2/conf/httpd.conf -k graceful"
    else
        server_restart_command="/home/rex/Desktop/Code/WordPress/Ytube/nginx/sbin/nginx -s reload"
    fi

    crontab $USER -l 2> /dev/null | {
        cat
        echo "0 0 1 * * $SUDO $LEGO_BIN --path=\"/home/rex/Desktop/Code/WordPress/Ytube/letsencrypt\" --tls --email=\"${email}\" ${domain_args} renew && $SUDO $server_restart_command"
...
EOF
    ...
    "$LEGO_BIN" --path "/home/rex/Desktop/Code/WordPress/Ytube/letsencrypt" --tls --email="${email}" ${domain_args} run
